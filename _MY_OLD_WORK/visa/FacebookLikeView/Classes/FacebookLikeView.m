...
NSString *const removeFBLikeButtonNotification = @"RemoveCoverButtonNotification";
...
-(void)removeLikeButton
{
//    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"hh" message:@"Hhh" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil];
//    [alert show];
//    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"fbLoggedIn"])
    if([[AppData sharedAppData].shopManager isLoggedInFb])
    {
//        _webView.hidden = YES;
        [[AppData sharedAppData].shopManager getVisaForLike];
        [[NSNotificationCenter defaultCenter] postNotificationName:removeFBLikeButtonNotification object:nil];
        [NSTimer scheduledTimerWithTimeInterval:1.0f target:self selector:@selector(removeFromSuperview) userInfo:nil repeats:NO];
...
- (void)initCommon {
    ...
    UITapGestureRecognizer *gr = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(removeLikeButton)];
    gr.delegate = self;
...
- (void)load {
    NSString *htmlFormat = [NSString stringWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"FacebookLikeView" ofType:@"html"] encoding:NSUTF8StringEncoding error:nil];
