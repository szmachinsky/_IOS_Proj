...
//    connection = [[NSURLConnection alloc] initWithRequest:request delegate:self startImmediately:YES]; // ivar
    connection = [[NSURLConnection alloc] initWithRequest:request delegate:self]; // ivar
    // Here is the trick
    NSLog(@"load:%@",requestString);
//    NSPort *port = [NSPort port];
//    NSRunLoop* rl = [NSRunLoop currentRunLoop]; // Get the runloop
//    [rl addPort:port forMode:NSDefaultRunLoopMode];
//    [connection scheduleInRunLoop:rl forMode:NSDefaultRunLoopMode];
    [connection start];
//   [rl runUntilDate:[NSDate dateWithTimeIntervalSinceNow:1]]; //1
}
