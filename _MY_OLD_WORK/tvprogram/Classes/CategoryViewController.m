...
//		self.tabBarItem.image = [UIImage imageNamed:@"t_categories.png"];
        
        CustomTabBarItem *tabItem = [[CustomTabBarItem alloc] initWithTitle:@"Категории" image:[UIImage imageNamed:@"t_categories.png"] tag:0];
        tabItem.customHighlightedImage=[UIImage imageNamed:@"t_categories.png"];
...
- (void)viewDidLoad
{
    ...
    backButton.tintColor = [SZUtils colorLeftButton];//[SZUtils color08];//[UIColor redColor];
...
//     
    ...
    topImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"white_top_line_5.png"]];
    topImage.frame = CGRectMake(0, 0, 320, 5);
    [self.view addSubview:topImage];

    bottomImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"white_bottom_line_5.png"]];
    bottomImage.frame = CGRectMake(0, 382, 320, 5);
...
//    mytableView.backgroundColor = [SZUtils color02];
    mytableView.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"blue_line_1.png"]];
    mytableView.frame = CGRectMake(0, 6, frame.size.width, 387 - 12);
///    mytableView.sectionIndexMinimumDisplayRowCount = 8;
//    mytableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    mytableView.separatorColor = [UIColor whiteColor];//[SZUtils color01]; 
    
    [self.view addSubview:mytableView];

    tableFrame = mytableView.frame; 
    lineFrame = bottomImage.frame;
    
//    mytableView.tableHeaderView = im1;
//    mytableView.tableFooterView = im2;
...
//          cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;            
        ...
        cell.textLabel.textColor = [UIColor whiteColor];
//        cell.textLabel.textColor = [UIColor colorWithRed:139./255. green:220./255. blue:221./255 alpha:1]; 
    }
    ...
    cell.imageView.image = [ModelHelper categoryImage:indexPath.row];
//    cell.imageView.frame = CGRectMake(0, 0, 44, 44);
//    float s = cell.imageView.frame.size.height;
...
{
    if (indexPath.row <= 7) {
        cell.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"blue_line_1.png"]];  
    } else {
//        cell.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"title_channel_1.png"]];  
    }
}
...
- (void)_bannerViewDidLoadAd:(ADBannerView *)banner  {
	if (!adViewVisible) {
...
//        mytableView.frame = CGRectMake(0.0f, CGRectGetMaxY(bannerView.frame), CGRectGetWidth(mytableView.frame), CGRectGetMinY(timeScrolling.frame) - CGRectGetMaxY(bannerView.frame));
        ...
        CGRect fr = mytableView.frame;
        fr.size.height -= bannerView.frame.size.height;
        mytableView.frame = fr;
...
//        
        fr = bottomImage.frame;
        fr.origin.y -= bannerView.frame.size.height;
        bottomImage.frame = fr;
...
- (void)_bannerView:(ADBannerView *)banner didFailToReceiveAdWithError:(NSError *)error
{
    //iAds failed
	...
	if (adViewVisible) {
        ...
        [self.view sendSubviewToBack:bannerView];
        mytableView.frame = tableFrame;//CGRectMake(0.0f, 0.0f, CGRectGetWidth(mytableView.frame), mytableView.frame.size.height + bannerView.frame.size.height);
        bottomImage.frame = lineFrame;
        [UIView commitAnimations];
		adViewVisible = FALSE;
    }
}
...
- (void) _bannerViewInfoLoaded:(WPBannerView *)bannerView_
{
...
- (void)bannerViewDidLoadAd:(ADBannerView *)banner  {
	if (!adViewVisible) {
        ...
        mytableView.frame = frame1;
        
        CGRect fr = bottomImage.frame;
        fr.origin.y -= h;
        bottomImage.frame = fr;
